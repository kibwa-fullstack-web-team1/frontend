 ...
 * 일일 질문을 가져옵니다.
 * @param {string} userId - 사용자 ID
 * @returns {Promise<object>} 일일 질문 데이터
 */
export const getDailyQuestion = async (userId) => {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.detail || `일일 질문 조회 실패: ${response.status}`);
    }

    const data = await response.json();
    return data;
  } catch (error) {
    console.error('일일 질문 조회 실패:', error);
    throw error;
  }
};
...
export const submitVoiceAnswer = async (questionId, userId, audioBlob) => {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.detail || `음성 답변 제출 실패: ${response.status}`);
    }
